...
void txtData::loadGame(Player* player)
{
	...
	//룬
	int rune = _wtoi(loadDataString2(fileName, L"player", L"rune", 128));
	player->setRune(rune);

	//트랩레벨
	int bearLevel = _wtoi(loadDataString2(fileName, L"player", L"bear", 128));
	player->setBearLevel(bearLevel);

	int slowLevel = _wtoi(loadDataString2(fileName, L"player", L"slow", 128));
	player->setSlowLevel(slowLevel);
}
...
void txtData::saveGame(Player* player)
{
	//룬
	WCHAR rune[128];
	swprintf_s(rune, L"%d", player->getRune());
	addData(L"player", L"rune", rune);

	//트랩 레벨
	WCHAR bearLevel[128];
	swprintf_s(bearLevel, L"%d", player->getBearLevel());
	addData(L"player", L"bear", bearLevel);

	WCHAR slowLevel[128];
	swprintf_s(slowLevel, L"%d", player->getSlowLevel());
	addData(L"player", L"slow", slowLevel);
